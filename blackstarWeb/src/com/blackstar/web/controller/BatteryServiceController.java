	  ...
	  public String  batteryservice(@RequestParam(required = true) Integer operation, @RequestParam(required = true) String idObject, ModelMap model)
	  {
		  try
		  {
			  if(operation!= null && idObject!=null)
			  {
				  ...
				  //si la operation es 4, el idObject es el id del batteryservice
		  		  if(operation==1)
		  		  {
		  			  ...
		  			  {	
		  				  ...
		  				  model.addAttribute("serviceOrder", new BatteryServiceDTO());
		  			  }
		  		  }
		  		  else if( operation==2)
		  		  {
		  		  ...
		  		  {
		  			  Integer idOrderService = Integer.parseInt(idObject);
		  			  BatteryServiceDTO batteryService = service.getBateryService(idOrderService);
		  			  
		  			  OrderserviceDTO serviceOrderDetail = null;
		  			  serviceOrderDetail = service.getServiceOrderByIdOrNumber(batteryService.getServiceOrderId(),"");
		  			  			  
		  			  if(serviceOrderDetail.getPolicyId()!=0)
		  			  {	
		  				  Policy policy = this.daoFactory.getPolicyDAO().getPolicyById(serviceOrderDetail.getPolicyId());
		  				  model.addAttribute("policyDetail", policy);
		  				  model.addAttribute("serviceOrder", batteryService);
		  				  model.addAttribute("serviceOrderDetail", serviceOrderDetail);
		  			  }  
		  		  }
	  		  }
