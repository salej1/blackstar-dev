...
public static void AssignIssue(Integer issueId, String asignee, String sender){
	...
	try {
		...
		da.executeUpdate(String.format("CALL AssignIssue('%s', '%s', '%s', '%s')", issueId, asignee, sender, "Dashboard"));
...
private static void SendIssueAssignationEmail(Integer issueId, String asignee, String sender){
	...
	String subject = String.format("Acción requerida por ", issue.getCreatedBy());
	StringBuilder bodySb = new StringBuilder();
	...
	bodySb.append(String.format("\r\n\r\n Asignado por: %s", issue.getCreatedBy()));
	bodySb.append(String.format("\r\n\r\n Fecha: %s", sdf.format(issue.getCreated())));
	bodySb.append(String.format("\r\n\r\n Accion requerida: %s", issue.getTitle()));
	bodySb.append(String.format("\r\n\r\n Detalles: %s", issue.getDetail()));
	bodySb.append(String.format("\r\n\r\n Proyecto: %s", issue.getProject()));
	bodySb.append(String.format("\r\n\r\n Cliente: %s", issue.getCustomer()));

	mail.sendEmail(issue.getCreatedBy(), to, subject, bodySb.toString());
}
}